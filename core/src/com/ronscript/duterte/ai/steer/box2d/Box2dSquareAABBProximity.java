...
public class Box2dSquareAABBProximity implements Proximity<Vector2>, QueryCallback {
	...
	public boolean reportFixture (Fixture fixture) {
		if(fixture.getBody().getUserData() instanceof  SteeringAgent) {
			SteeringAgent agent = (SteeringAgent) fixture.getBody().getUserData();
			if (agent != owner && accept(agent)) {
				if (behaviorCallback.reportNeighbor(agent)) neighborCount++;
			}
		}
		return true;
	}
