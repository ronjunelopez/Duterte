...
public class TiledRaycastCollisionDetector implements RaycastCollisionDetector<Vector2> {
    ...
    public TiledRaycastCollisionDetector(NavigationMap map) {
        this.map = map;
    }
    ...
    public boolean collides(Ray<Vector2> ray) {
        ...
        int ystep = (y0 < y1 ? 1 : -1);
        for (int x = x0; x <= x1; x++) {
            TiledNode tile = steep ? map.getNodeAt(y, x) : map.getNodeAt(x, y);
            if (!tile.walkable) return true; // We've hit a wall
            error += deltay;
